...
class ConfigurationView(BaseView):
        ...
        def index(self):
                ...
                if form.validate_on_submit():
                        ...
                        # If re-configuring file, delete previous version
                        if os.path.isfile("storage/config/" + object_id + ".yml"):
                                os.remove("storage/config/" + object_id + ".yml")
...
#    Lot database view customizations

class LotView(ModelView):
        form_excluded_columns = ['total_spots', 'available_spots', 'percentage', 'spots']
        can_create = True
        can_delete = True

#    Spot database view customizations

class SpotView(ModelView):
        can_create = False
        can_edit = False
...
admin.add_view(LotView(Lot, db.session, category = "Database"))
admin.add_view(SpotView(Spot, db.session, category = "Database"))
...
def database_import(avail_list, lot_id):
        ...
        with application.app_context():
                ...
                for x in avail_list:
                        ...
                        imported = Spot(availability = x, lot_location = lot_id, spot_id = spot_increment)
                        db.session.add(imported)
                        spot_increment = spot_increment + 1
                db.session.commit()
...
#       Init route [Retrieve IP address]

def init():
        global ip_address
        ip_address = input("\nEnter the IP address of the machine running the Camera server: " )
